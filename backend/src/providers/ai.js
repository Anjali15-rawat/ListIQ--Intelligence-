export const ai = {
  ...
  // Used for market/competitor analysis — returns flexible JSON (no overallScore required)
  async generateMarketResponse(systemPrompt, userPrompt) {
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) throw new Error('OPENAI_API_KEY is not set in .env');

    console.log('🤖 [AI] Calling OpenAI gpt-4o-mini (market analysis)...');
